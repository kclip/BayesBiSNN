...
def next(hdf5_group, batch_size=32, chunk_size=500, T=80, n_classes=10, n_inputs=[2, 26, 26], dt=1000):
...
def next_all(hdf5_group, chunk_size=500, T=80, n_classes=10, n_inputs=[2, 26, 26], dt=1000):
...
def get_event_slice(times, addrs, start_time, end_time, chunk_size, n_inputs=[2, 26, 26], dt=1000):
    try:
...
def chunk_evs_pol(times, addrs, deltat=1000, n_inputs=[2, 26, 25]):
    t_start = times[0]
    ...
    for i, t in enumerate(ts):
        ...
        if idx_end > idx_start:
            ...
            try:
                np.add.at(chunks, (i, ((1 + ee[:, 2]) / 2).astype(np.int), ee[:, 0], ee[:, 1]), 1)  # i, pol, x, y
            except:
                print(i, t, addrs.shape, idx_start, idx_end)
                print(((1 + ee[:, 2]) / 2).astype(np.int), ee[:, 0], ee[:, 1])
                raise IndexError
        idx_start = idx_end
    return chunks
