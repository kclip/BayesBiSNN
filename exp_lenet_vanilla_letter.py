...
for inputs, labels in train_iterator:
    ...
    for t in range(burnin, T):
        ...
        with torch.no_grad():
            gradients_means.append(torch.mean(torch.abs(torch.cat([w.grad.flatten() for w in model.get_trainable_parameters()]))).numpy())
            gradients_stds.append(torch.std(torch.abs(torch.cat([w.grad.flatten() for w in model.get_trainable_parameters()]))).numpy())
